...
def main():
    ...
    if decrypt:
        disa = cryptoUnwrap(disa, saveType, saveId, keyEngine.getKeySdDecrypt())
        if disa is None:
            exit(1)
        ...
        if saveId is None:
            ...
            if key is None:
                ...
                digest = hashlib.sha256(
                    getDigestBlock(saveType, saveId, header)
                ).digest()
                import cmac
    ...
    (
        ...
        activeTable,
        tableHash,
    ) = struct.unpack('<III4xQQQQQQQQQQQB3x32s116x', header)
    ...
    partADescriptor = partTable[
        partADiscriptorOff : partADiscriptorOff + partADiscriptorSize
    ]
    ...
    if hasData:
        partBDescriptor = partTable[
            partBDiscriptorOff : partBDiscriptorOff + partBDiscriptorSize
        ]
    ...
    # Reads SAVE header
    SAVE, ver, filesystemHeaderOff, imageSize, imageBlockSize, x00 = struct.unpack(
        '<IIQQII', partAInner[0:0x20]
    )
    ...
    # Parses hash tables
    dirHashTable = savefilesystem.getHashTable(
        fsHeader.dirHashTableOff, fsHeader.dirHashTableSize, partAInner
    )

    fileHashTable = savefilesystem.getHashTable(
        fsHeader.fileHashTableOff, fsHeader.fileHashTableSize, partAInner
    )
    ...
    dirList = savefilesystem.getDirList(fsHeader, partAInner, dataRegion, fat)

    print("Directory list:")
    for i in range(len(dirList)):
        dirList[i].printEntry(i)

    fileList = savefilesystem.getFileList(fsHeader, partAInner, dataRegion, fat)
